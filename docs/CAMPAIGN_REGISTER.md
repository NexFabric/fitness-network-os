...
| **2026-08-21** audit hardening | `2d2be60` (`fix/p0-p1-audit`) | finance double match prevention (`already_matched`), audit trail on staff roles, request log `tenant_id`, centralized `formatApiError`, reception search race condition guard (`isCurrent`), StoryBrand Hero copy & de-jargonized pricing, 44px mobile touch targets & iOS safe-area insets | do not reopen financial reconciliation matching or reception search race condition |
