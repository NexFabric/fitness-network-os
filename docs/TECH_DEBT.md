...
| TD-5 | `eslint` 9 → 10 (with `@eslint/js` 10) | Dependabot PR #71, dropped from the batch 2026-08-16 | M | `@eslint/js@10.0.1` declares `peerOptional eslint@^10`, but the workspaces pin `eslint@9.39.5`, so npm cannot resolve the two together. Bumping `@eslint/js` alone is incoherent — the eslint core has to move with it, and an eslint major usually brings new rule failures (see TD-1). Do both in one dedicated PR. |
