...
from sqlalchemy import String, ForeignKey, Float, Integer, Text, DateTime
...
class Opportunity(TenantMixin, Base):
    ...
    lead_id: Mapped[Optional[UUID]] = mapped_column(ForeignKey("leads.id"), nullable=True)
    member_id: Mapped[Optional[UUID]] = mapped_column(ForeignKey("members.id"), nullable=True)
...
class RetentionCockpit(TenantMixin, Base):
    ...
    member_id: Mapped[UUID] = mapped_column(ForeignKey("members.id"), nullable=False, unique=True)
