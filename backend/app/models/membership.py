...
class Membership(TenantMixin, Base):
    ...
    scheduled_cancellation_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    price_snapshot: Mapped[int | None] = mapped_column(Integer, nullable=True)
    price_snapshot_currency: Mapped[str | None] = mapped_column(
...
class Entitlement(TenantMixin, Base):
    ...
    _model_table_args = (
        ...
        ForeignKeyConstraint(
            ["tenant_id", "membership_id"], ["memberships.tenant_id", "memberships.id"]
        ),
    )
...
class MembershipFreeze(TenantMixin, Base):
    ...
    _model_table_args = (
        ...
        Index(
            ...
            "membership_id",
            unique=True,
            postgresql_where=text("actual_end_date IS NULL"),
        ),
    )
    start_date: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False
...
class MembershipStatusHistory(TenantMixin, Base):
    ...
    changed_by_user_id: Mapped[UUID | None] = mapped_column(
        Uuid, nullable=True
    )  # ID of user/staff who made the change
...
class MembershipRenewal(TenantMixin, Base):
    ...
    status: Mapped[str] = mapped_column(
        String, nullable=False, default=RenewalStatus.PENDING
    )
    price_snapshot: Mapped[int | None] = mapped_column(Integer, nullable=True)
    price_snapshot_currency: Mapped[str | None] = mapped_column(
        String(3), nullable=True
