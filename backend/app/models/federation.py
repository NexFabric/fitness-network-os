...
class ComplianceRecord(Base, TenantMixin):
    ...
    audit_date: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=lambda: datetime.now(UTC))
...
class NetworkAlert(Base):
    ...
    target_tenant_id: Mapped[UUID | None] = mapped_column(ForeignKey("tenants.id"), nullable=True)
