...
async def test_tenant_context_isolation(pg_engine, pg_session_maker):
    ...
    async with pg_engine.begin() as conn:
        ...
        await conn.execute(
            text("ALTER TABLE dummy_tenant_items ENABLE ROW LEVEL SECURITY;")
        )
        await conn.execute(
            text("DROP POLICY IF EXISTS tenant_isolation_policy ON dummy_tenant_items;")
        )
        await conn.execute(
            text(
        ...
        await conn.execute(
            text("ALTER TABLE dummy_tenant_items FORCE ROW LEVEL SECURITY;")
        )
    ...
    async with pg_session_maker() as session:
        ...
        try:
            ...
            assert (
                "new row violates row-level security policy" in str(e)
                or "row violates row-level security" in str(e)
                or "violates row-level security policy" in str(e)
            )
        ...
        res = await session.execute(
            select(DummyTenantItem).where(DummyTenantItem.name == "Item B1")
        )
        ...
        assert upd_res.rowcount == 0, (
            "Should not be able to update another tenant's row"
        )

        # Test DELETE on Tenant B's data
        del_res = await session.execute(
            delete(DummyTenantItem).where(DummyTenantItem.id == b_id)
        )
        assert del_res.rowcount == 0, (
            "Should not be able to delete another tenant's row"
        )
    ...
    async with pg_session_maker() as session:
        ...
        try:
            ...
            await session.rollback()
            assert "new row violates row-level security policy" in str(
                e
            ) or "violates row-level security" in str(e)
