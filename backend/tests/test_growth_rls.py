...
async def test_growth_models_tenant_context_isolation(rls_session_maker):
    ...
    async with rls_session_maker() as session:
        ...
        # Test Lead
        lead = Lead(tenant_id=tenant_a, first_name="John", last_name="Doe", status="NEW")
        session.add(lead)
        await session.commit()
        await session.refresh(lead)
        
        # Test Opportunity
        opportunity = Opportunity(tenant_id=tenant_a, lead_id=lead.id, stage="PROSPECTING")
        ...
        member_id = uuid4()
        cockpit = RetentionCockpit(tenant_id=tenant_a, member_id=member_id, health_score=85)
        session.add(cockpit)
