...
def access_session_maker(access_engine):
    return async_sessionmaker(access_engine, class_=AsyncSession, expire_on_commit=False)
...
async def test_signing_key_tenant_isolation(access_session_maker):
    ...
    async with access_session_maker() as session:
        # Create ACTIVE key
        key1 = SigningKey(tenant_id=tenant_id, kid="qr-2026-08-a", status=KeyStatus.ACTIVE, key_material="secret_abc")
        session.add(key1)
        
        # Create VERIFY_ONLY key
        key2 = SigningKey(tenant_id=tenant_id, kid="qr-2026-07", status=KeyStatus.VERIFY_ONLY, key_material="secret_xyz")
        session.add(key2)
        
        await session.commit()
        
        result = await session.execute(select(SigningKey).filter_by(tenant_id=tenant_id))
...
async def test_device_and_checkin_isolation(access_session_maker):
    ...
    async with access_session_maker() as session:
        ...
        member = Member(tenant_id=tenant_id, member_number="A-001", first_name="Access", last_name="User")
        session.add(member)
        await session.commit()
        ...
        # Assertions
        devices_db = (await session.execute(select(Device).filter_by(tenant_id=tenant_id))).scalars().all()
        assert len(devices_db) == 1
        assert devices_db[0].status == DeviceStatus.ONLINE
        assert "QR_SCAN" in devices_db[0].capabilities
        
        attempts_db = (await session.execute(select(AccessAttempt).filter_by(tenant_id=tenant_id))).scalars().all()
        assert len(attempts_db) == 1
        assert attempts_db[0].status == AccessStatus.GRANTED
        
        checkins_db = (await session.execute(select(Checkin).filter_by(tenant_id=tenant_id))).scalars().all()
        assert len(checkins_db) == 1
        assert checkins_db[0].member_id == member.id
        
        snapshots_db = (await session.execute(select(OfflineSnapshot).filter_by(tenant_id=tenant_id))).scalars().all()
