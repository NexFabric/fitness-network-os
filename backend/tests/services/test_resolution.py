...
async def setup_tenant(db_session):
    ...
    t_id = uuid4()
    tenant = Tenant(
        id=t_id, name="Test Tenant", organization_id=org.id, location_code=f"LOC-{t_id}"
    )
...
async def setup_base_data(db_session, setup_tenant):
    ...
    plan = Plan(id=uuid4(), tenant_id=tenant_id, name="Test Plan")
    db_session.add(plan)
...
@pytest.mark.asyncio
async def test_activate_scheduled_memberships(
    db_session, pg_session_maker, setup_tenant, setup_base_data
):
    tenant_id = setup_tenant.id
...
@pytest.mark.asyncio
async def test_process_expirations(
    db_session, pg_session_maker, setup_tenant, setup_base_data
):
    tenant_id = setup_tenant.id
    member, _plan, pv = setup_base_data
...
@pytest.mark.asyncio
async def test_process_renewals(
    db_session, pg_session_maker, setup_tenant, setup_base_data
):
    tenant_id = setup_tenant.id
    ...
    stmt = select(MembershipPeriod).where(
        MembershipPeriod.membership_id == m.id, MembershipPeriod.is_active
    )
...
async def test_two_tenant_isolation(
    db_session, pg_session_maker, setup_tenant, setup_tenant_2, setup_base_data
):
...
@pytest.mark.asyncio
async def test_renewal_error_isolation(
    setup_base_data, pg_session_maker, setup_tenant, db_session
):
    tenant_id = setup_tenant.id
    member, _plan, pv = setup_base_data
    now = datetime.now(UTC)
    ...
    p1 = MembershipPeriod(
        membership_id=m1.id,
        tenant_id=tenant_id,
        start_date=m1.start_date,
        end_date=m1.end_date,
        is_active=True,
    )
    ...
    p2 = MembershipPeriod(
        membership_id=m2.id,
        tenant_id=tenant_id,
        start_date=m2.start_date,
        end_date=m2.end_date,
        is_active=True,
    )
