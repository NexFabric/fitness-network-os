...
async def setup_membership(pg_engine):
    ...
    async with async_session() as db:
        ...
        tenant = Tenant(id=tenant_id, name="Tenant A", location_code=f"TA-{uuid4().hex[:6]}", organization_id=org.id)
        db.add(tenant)
        await db.flush()
...
async def test_concurrent_freeze(pg_session_maker, setup_membership):
    ...
    async def freeze_op():
        async with pg_session_maker() as db:
            ...
            await db.execute(text(f"SET LOCAL app.current_tenant_id = '{setup_membership['tenant_id']}'"))
    ...
    if isinstance(failures[0], ValueError):
        assert "cannot be frozen" in str(failures[0]) or "already has an active freeze" in str(failures[0])
...
async def test_concurrent_unfreeze(pg_session_maker, setup_membership):
    ...
    if isinstance(failures[0], ValueError):
        assert "not frozen" in str(failures[0]) or "No active freeze record found" in str(failures[0])
