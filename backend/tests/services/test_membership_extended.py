...
async def setup_data(pg_session_maker):
    ...
    async with pg_session_maker() as session:
        member = Member(tenant_id=tenant_id, member_number="M-TEST1", first_name="T1", last_name="T1", status="ACTIVE")
        session.add(member)
        plan = Plan(tenant_id=tenant_id, name="Test Plan")
        session.add(plan)
        await session.flush()
        
        pv1 = PlanVersion(tenant_id=tenant_id, plan_id=plan.id, version=1, price_amount_minor=10000, billing_cycle_months=1, is_published=True)
        pv2 = PlanVersion(tenant_id=tenant_id, plan_id=plan.id, version=2, price_amount_minor=12000, billing_cycle_months=1, is_published=True)
...
async def test_start_and_overlap(pg_session_maker, setup_data):
    ...
    async with pg_session_maker() as session:
        ...
        with pytest.raises(ValueError, match="already has an active or scheduled membership"):
...
async def test_frozen_cancel(pg_session_maker, setup_data):
    ...
    async with pg_session_maker() as session:
        ...
        stmt = select(MembershipFreeze).where(MembershipFreeze.membership_id == membership_id)
