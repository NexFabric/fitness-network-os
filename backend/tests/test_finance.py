...
def finance_session_maker(finance_engine):
    return async_sessionmaker(finance_engine, class_=AsyncSession, expire_on_commit=False)
...
async def test_payment_allocation_math(finance_session_maker):
    ...
    async with finance_session_maker() as session:
        ...
        alloc_result = await session.execute(select(PaymentAllocation).filter_by(invoice_id=invoice.id))
