...
**Progress truth (2026-08-15):** Phase **27 / 27.1 / 27.2 / 27.3 / 27.4 + Waves 1–3 (P0/P1 MVP Product Closure)** is on branch `feat/production-readiness-p0-and-dr` at `a4921d0`, Alembic head `x6f7a8b9c0d1`. PR **#55** and PR **#57** merged privileged MFA, private S3 reports, Prometheus metrics, staff provisioning. Waves 1–3 added legal pages, `/me` self-service, forensic access snapshots, reception workspace (`/reception`), server-side KPI engine (`/dashboard/kpis`), CSV migration pipeline (`/import`), payment attempts & dunning, and tenant onboarding state machine. Test suite on real PostgreSQL is **340 passed · 1 skipped**, Playwright 36/36. Phase **26 CORE MVP EXIT GATE remains NOT PASSED** — real-bucket S3 proof and independent pentest are missing; **not production-ready**.
